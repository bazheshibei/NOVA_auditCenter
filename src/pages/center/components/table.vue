  ...
  methods: {
    ...
    submit(title) {
      ...
      if (node_content_type === 'time' || node_content_type !== 'content') {
        ...
        node.timeType = 2
        if (node.auditSplitLength === node.audit_process_record.length) {
          node.audit_process_record.push(`【${_getTime} ${employeename}】变更节点信息为${change_plan_time}；原因：${change_remaark}`) // 审核过程记录
        } else {
          node.audit_process_record[node.audit_process_record.length - 1] = `【${_getTime} ${employeename}】变更节点信息为${change_plan_time}；原因：${change_remaark}`
        }
      }
